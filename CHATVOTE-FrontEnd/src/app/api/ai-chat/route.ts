              ...
              const promise = (async () => {
                ...
                if (perCandidateResults.size === 0 && manifestoResults.length === 0) {
                  const webSearchHint = features.includes('perplexity')
                    ...
                    : '';
                  return { results: [] as SearchResult[], count: 0, message: `Aucun document trouvé dans la base documentaire.${webSearchHint}` };
                }
...
${resolvedFeatures.includes('perplexity') ? `- **⚠️ Recherche web complémentaire (OBLIGATOIRE — RÈGLE LA PLUS IMPORTANTE)** :
