...
export async function searchDataGouv(query: string, limit = 5): Promise<DataGouvDataset[]> {
...
export function buildTools(enabledFeatures: string[] | undefined, candidateIds: string[] = [], candidateNames: Map<string, string> = new Map(), selectedCandidateIds: string[] = []) {
...
export async function runChatPipeline(params: ChatPipelineParams): Promise<ChatPipelineResult> {
  const {
    question,
    partyIds,
    candidateIds = [],
  ...
  const systemPrompt = buildSystemPrompt({
    ...
    candidateIds,
    candidateContext,
    searchCandidateIds: selectedCandidateIds.length > 0 ? selectedCandidateIds : candidateIds,
  ...
  // Build tools
  const tools = buildTools(enabledFeatures, candidateIds, candidateNames, selectedCandidateIds);
  ...
  if (!params.model) {
    if (!process.env.GOOGLE_GENERATIVE_AI_API_KEY) {
      console.warn('[chat-pipeline] GOOGLE_GENERATIVE_AI_API_KEY missing, falling back to Scaleway');
      model = scalewayChat;
    }
  }
  ...
  for (const step of result.steps) {
    for (const tc of step.toolCalls) {
      toolCalls.push({
        ...
        args: ((tc as any).args ?? (tc as any).input ?? {}) as Record<string, unknown>,
        resultPreview: '',
      });
    }
  }
  ...
  for (const step of result.steps) {
    for (const tr of step.toolResults) {
      const resultValue = (tr as any).result ?? (tr as any).output;
      const resultList: any[] = resultValue?.results ?? resultValue?.findings ?? [];
  ...
  return {
    ...
    output: result.text || result.steps.map((s) => s.text).filter(Boolean).join('\n'),
